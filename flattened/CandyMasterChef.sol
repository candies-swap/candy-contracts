...
// File: contracts\libs\ICandyReferral.sol
...
// File: contracts\CandySyrupBar.sol

pragma solidity 0.6.12;



// CandySyrupBar with Governance.
contract CandySyrupBar is BEP20('SyrupBar Token', 'SYRUP') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
...
// File: contracts\CandyMasterChef.sol
...
contract CandyMasterChef is Ownable {
    using SafeMath for uint256;
    ...
    event SetCandyReferral(address newReferral);
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            uint256 candyReward = multiplier
                ...
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accCandyPerShare = accCandyPerShare.add(
                candyReward.mul(1e12).div(pool.lpSupply)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accCandyPerShare = pool.accCandyPerShare.add(
            candyReward.mul(1e12).div(pool.lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(
        ...
        if (
            _amount > 0 &&
            address(candyReferral) != address(0) &&
            _referrer != address(0) &&
            _referrer != msg.sender &&
            _referrer != candyReferral.getReferrer(msg.sender)
        ) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accCandyPerShare).div(1e12).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeCandyTransfer(msg.sender, pending);
    ...
    function updateEmissionRate(uint256 _candyPerBlock) public onlyOwner {
        require(_candyPerBlock <= MAXIMUM_EMISSON_RATE, "Too high");
        massUpdatePools();
        candyPerBlock = _candyPerBlock;
    ...
    function setCandyReferral(ICandyReferral _candyReferral) public onlyOwner {
        require(
            address(candyReferral) != address(_candyReferral),
            "Already set"
        );
        candyReferral = _candyReferral;
        emit SetCandyReferral(address(_candyReferral));
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (
            address(candyReferral) != address(0) && referralCommissionRate > 0
        ) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                ...
                candyReferral.recordReferralCommission(
                    referrer,
                    commissionAmount
                );
